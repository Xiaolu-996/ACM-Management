...
public class UserController extends BaseController {
  ...
  @Autowired private BCryptPasswordEncoder bCryptPasswordEncoder; // 注入bcryct加密
  ...
  public Result<?> login(@RequestBody User userParam) {
    ...
    for (UserRole userRole : userRoles) {
      ...
      List<RolePermission> rolePermissions =
          permissionMapper.getRolePermissionByRoleId(userRole.getRoleId());
    ...
    // 对资源根据id进行排序
    LinkedHashSet<Permission> sortedSet =
  ...
  public Result<?> register(@RequestBody User user) {
    ...
    UserRole userRole =
        UserRole.builder().userId(userInfo.getId()).roleId(RoleEnum.USER.getRoleId()).build();
  ...
  public Result<?> save(@RequestBody User user) {
    ...
    UserRole userRole =
        UserRole.builder().userId(user.getId()).roleId(RoleEnum.USER.getRoleId()).build();
    userRoleMapper.insert(userRole);
  ...
  public Result<?> insertByUidAndOid(@PathVariable Integer oid) {
    // System.out.println("oid: "+oid);
    // System.out.println("uid: "+getUser().getId());
    System.out.println("time: " + DateUtil.formatDateTime(new Date()));
    int uid = getUser().getId();
    String time = DateUtil.formatDateTime(new Date());
    userOrderMapper.insertByUidAndOid(uid, oid, time);
    return Result.success();
  }

  // 获取每个人的提交数据
  @GetMapping("/personOrder")
  public Result<?> findPersonOrder(@RequestParam Integer id) {
    System.out.println("从前端获取到的ID是：" + id);
    System.out.println("到这里啦啦啦啦啦啦++++++++++++++++++++++++++++++");
    List<Order> personOrder = userOrderMapper.findPersonOrder();
    List<Order> orderResult = new ArrayList<>();

    for (Order value : personOrder) {
      if (id.equals(value.getUid())) {
        orderResult.add(value);
      }
    }
  ...
  public Result<?> findPage(
    ...
    for (User record : userPage.getRecords()) {
      ...
      List<Integer> roleIds =
          roles.stream().map(UserRole::getRoleId).distinct().collect(Collectors.toList());
